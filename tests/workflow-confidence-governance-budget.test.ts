...
describe("workflow confidence: governance + budget", () => {
  ...
  it("codifies budget scope as agent-level in table models", async () => {
    ...
    // `createProject`/`createIssue` inputs do not carry budget fields today.
    expect(projectColumns.includes("monthlyBudgetUsd")).toBe(false);
    expect(projectColumns.includes("usedBudgetUsd")).toBe(false);
    expect(issueColumns.includes("monthlyBudgetUsd")).toBe(false);
    expect(issueColumns.includes("usedBudgetUsd")).toBe(false);
    // Agent table explicitly carries budget accounting.
